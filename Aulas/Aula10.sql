...
/*TRABALHANDO COM INTERVALOS, podem ser usados para, por exe, alterar a valor por um determinado tempo*/
SELECT *
FROM tb_promocao;

SELECT
  NUMTODSINTERVAL(1.5,'DAY') AS DIA,
  NUMTODSINTERVAL(3.25,'HOUR') AS HORA,
  NUMTODSINTERVAL(5,'MINUTE') AS MINUTO,
  NUMTODSINTERVAL(10.123456789,'SECOND') AS SEGUNDO
FROM dual;

SELECT
  NUMTOYMINTERVAL(1.5,'YEAR') AS ANO,
  NUMTOYMINTERVAL(3.25,'MONTH') AS MESES,
FROM dual;
